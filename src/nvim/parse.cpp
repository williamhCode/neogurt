...
static const std::unordered_map<std::string_view, UiEventFunc> uiEventFuncs = {
  ...
  {"set_title", [](const msgpack::object& args, RedrawState& state) {
    state.currEvents().emplace_back(args.as<SetTitle>());
  }},

  {"set_icon", [](const msgpack::object& args, RedrawState& state) {
    state.currEvents().emplace_back(args.as<SetIcon>());
  }},

  {"mode_info_set", [](const msgpack::object& args, RedrawState& state) {
    state.currEvents().emplace_back(args.as<ModeInfoSet>());
  }},
  ...
  {"win_viewport_margins", [](const msgpack::object& args, RedrawState& state) {
    ...
    state.currEvents().emplace_back(args.as<WinViewportMargins>());
  }},
